"""Generate the deterministic v0.1 demonstration dataset."""
...
def base_ticket(number: int, sprint_number: int, status: str) -> dict:
    sprint_starts = {1: date(2026, 6, 8), 2: date(2026, 6, 22), 3: date(2026, 7, 6)}
...
def build_commits() -> list[dict]:
    ...
    for number in range(1, 31):
        ...
        ticket_number = {1: number, 2: number + 7, 3: number + 14}[sprint_number]
        committed = datetime(2026, 6, 10, 10, tzinfo=UTC) + timedelta(days=number * 2)
...
def build_pull_requests() -> list[dict]:
    pull_requests = []
    ticket_numbers = [3, 6, 10, 14, 18, 22, 27, 32, 36, 38, 42, 47]
    for number, ticket_number in enumerate(ticket_numbers, start=1):
        sprint_number = (number - 1) // 4 + 1
        status = "merged"
        if number in {7, 10, 11, 12}:
            status = "open"
        created = datetime(2026, 6, 11, 11, tzinfo=UTC) + timedelta(days=number * 3)
        pull_requests.append(
            {
                ...
                "created_at": iso_datetime(created),
                "merged_at": (
                    iso_datetime(created + timedelta(days=1)) if status == "merged" else None
                ),
...
def build_builds(commits: list[dict]) -> list[dict]:
    ...
    builds = []
    starts = {
        1: datetime(2026, 6, 12, 14, tzinfo=UTC),
        2: datetime(2026, 6, 26, 14, tzinfo=UTC),
        3: datetime(2026, 7, 8, 14, tzinfo=UTC),
    }
    for number, status in enumerate(statuses, start=1):
        sprint_number = (number - 1) // 4 + 1
        started = starts[sprint_number] + timedelta(days=(number - 1) % 4)
        ...
        builds.append(
            {
                ...
                "branch": "main" if number % 4 == 0 else f"feature/sprint-{sprint_number}",
